    ...
    <div
        ...
        <div class="grid grid-cols-12 pt-4">
            <div class="col-span-12  md:col-span-6 p-1">
                ...
                <button v-if="encomenda.status != 'ENTREGUE' && encomenda.status == 'EM_TRANSITO'"
                    @click="tentativaDeEntrega(encomenda.id)" type="button"
...
const tentativaDeEntrega = async (id) => {
    const { data, error } = await useFetch(`${api}/encomendas/${id}/status/tentativaEntrega`, {
        method: 'POST',
        headers: { "Authorization": `Bearer ${authStore.token}` }
    })
    if (data) {
        toast.success('Tentativa de entrega registada com sucesso!')
        refresh()
    }
}
