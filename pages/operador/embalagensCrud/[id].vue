...
const { data: embalagem, error, refresh } = await useFetch(`${api}/embalagensTransporte/${id}`, { headers: { "Authorization": `Bearer ${authStore.token}` } })
...
const desassociar = async (sensorID) => {
    await useFetch(`${api}/embalagensTransporte/${id}/sensor/${sensorID}`, {
        method: 'DELETE',
        headers: { "Authorization": `Bearer ${authStore.token}` }
    }).then(() => {
