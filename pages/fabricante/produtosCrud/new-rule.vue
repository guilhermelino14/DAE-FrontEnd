            ...
            <div class="col-span-12 md:col-span-11 sm:col-span-10">
                ...
                <div class="items-center justify-between lg:flex">
                    <div class="mb-4 lg:mb-0">
                        <h3 class="mb-2 text-xl font-bold text-gray-900 dark:text-white">Criar nova regra
                        </h3>
                    </div>
                </div>
            </div>
        ...
        <form @submit.prevent="adicionar" class="max-w-sm mx-auto">
            <div>
                <label for="countries" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Selecionar o
                    tipo de
                    Sensor</label>
                <select id="countries" v-model="newRule.typeOfSensor"
            ...
            <div class="mb-5">
                <label for="base-input"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Valor Minimo:</label>
                <input type="number" v-model="newRule.val_min" required
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            </div>
            <div class="mb-5">
                <label for="base-input"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Valor Maximo:</label>
                <input type="number" v-model="newRule.val_max" required
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            </div>
...
const adicionar = async () => {
    loading.value = true
    try {
        const response = await useFetch(`${api}/produtos/${id}/createRole`, {
            method: 'POST',
        ...
        if (response.status.value === "success") {
            ...
            navigateTo("/fabricante/produtosCrud/"+id)
        }
